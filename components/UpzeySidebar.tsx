...
interface UpzeySidebarProps {
  activeMenu: string;
  onMenuChange: (menu: string) => void;
}

const UpzeySidebar: React.FC<UpzeySidebarProps> = ({ activeMenu, onMenuChange }) => {
  const menuItems = [
    { id: 'image', name: 'Image', icon: 'fa-image', iconType: 'fa' as const },
    { id: 'video', name: 'Video', icon: 'fa-video', iconType: 'fa' as const },
  ];
  ...
  return (
    <>
      ...
      <aside className="hidden md:flex fixed left-24 top-16 bottom-0 w-64 bg-black/80 backdrop-blur-sm border-r border-cyan-500/20 z-30">
      ...
      {/* Mobile: Footer Menu */}
      <footer className="md:hidden fixed bottom-0 left-0 right-0 bg-black/90 backdrop-blur-sm border-t border-cyan-500/20 z-40 safe-area-inset-bottom">
