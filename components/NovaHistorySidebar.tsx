...
interface NovaHistorySidebarProps {
  onSelectImage: (url: string) => void;
  onClose: () => void;
}

export const NovaHistorySidebar: React.FC<NovaHistorySidebarProps> = ({ onSelectImage, onClose }) => {
  ...
  return (
    ...
    >
      <div className="fixed top-0 right-0 h-full w-full md:w-96 bg-gray-900/50 backdrop-blur-lg animate-in slide-in-from-right-full duration-300 z-[60]" onClick={(e) => e.stopPropagation()}>
        <div className="flex flex-col h-full glass border-l border-violet-500/20">
          <div className="flex items-center justify-between p-4 border-b border-gray-800">
            ...
            <div className="flex items-center gap-2">
              <button
                ...
                disabled={isLoading}
                className={`text-gray-500 hover:text-violet-400 transition-colors p-2 rounded-lg hover:bg-gray-800 disabled:opacity-50 disabled:cursor-not-allowed ${isLoading ? 'animate-spin' : ''}`}
          ...
          {selectedCount > 0 && (
            <div className="px-4 py-3 border-b border-gray-800 bg-violet-500/10">
              <button
                onClick={handleDownloadSelected}
                className="w-full py-2 px-4 bg-violet-600 hover:bg-violet-500 text-white font-semibold rounded-lg transition-colors flex items-center justify-center gap-2"
              >
          ...
          <div className="flex-1 overflow-y-auto p-4">
            {previewPrediction ? (
              ...
              <div className="flex flex-col h-full">
                <button
                  onClick={handleBack}
                  className="mb-4 flex items-center gap-2 text-gray-400 hover:text-violet-400 transition-colors text-sm font-medium"
                >
                  ...
                  {previewPrediction.outputs && previewPrediction.outputs.length > 0 ? (
                    <>
                      ...
                      <div className="mt-4 w-full space-y-2">
                        ...
                        {previewPrediction.input?.aspect_ratio && (
                          <div className="flex items-center justify-between text-sm">
                            <span className="text-gray-400">Aspect Ratio:</span>
                            <span className="text-gray-300">{previewPrediction.input.aspect_ratio}</span>
                          </div>
                        )}
                        {previewPrediction.input?.resolution && (
                          <div className="flex items-center justify-between text-sm">
                            <span className="text-gray-400">Resolution:</span>
                            <span className="text-gray-300 uppercase">{previewPrediction.input.resolution}</span>
                          </div>
                        )}
                        ...
                        {previewPrediction.input?.prompt && (
                          <div className="flex flex-col gap-1 text-sm">
                            <span className="text-gray-400">Prompt:</span>
                            <span className="text-gray-300 text-xs">{previewPrediction.input.prompt}</span>
                          </div>
                        )}
                        <div className="flex items-center justify-between text-sm">
                            ...
                            <div
                              ...
                              className={`relative bg-gray-800/50 rounded-lg overflow-hidden border transition-all ${
                                prediction.status === 'completed' && prediction.outputs && prediction.outputs.length > 0
                                  ? `cursor-pointer hover:border-violet-500/50 ${selectedIds.has(prediction.id) ? 'border-violet-500 bg-violet-500/10' : 'border-gray-700'}`
                            ...
                            >
                                ...
                                >
                                  <div className={`w-5 h-5 rounded border-2 flex items-center justify-center transition-colors ${
                                    selectedIds.has(prediction.id)
                                      ...
                                      : 'bg-gray-800/80 border-gray-600 hover:border-violet-500'
                                  }`}>
                                    {selectedIds.has(prediction.id) && (
                              ...
                              {prediction.outputs && prediction.outputs.length > 0 && prediction.status === 'completed' ? (
                                ...
                                <div className="p-8 flex flex-col items-center justify-center bg-gray-900/50">
                                  ...
                                  {prediction.status === 'processing' && (
                                    <svg className="animate-spin h-6 w-6 text-violet-400 mt-4" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                                      <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle>
                                      <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
                                    </svg>
                                  )}
                              ...
                              <div className="p-3">
                                ...
                                {prediction.input?.prompt && (
                                  <p className="text-xs text-gray-500 mt-1 line-clamp-1">{prediction.input.prompt}</p>
                                )}
                              </div>
                            </div>
                          ))}
