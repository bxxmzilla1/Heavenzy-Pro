...
import NudifySidebar from './NudifySidebar';
import { ImageMode, VideoMode } from './NudifyModes';
...
export const NudifyPage: React.FC = () => {
  ...
  const NudifyHistorySidebar: React.FC<{ onSelectImage: (url: string) => void; onClose: () => void }> = ({ onSelectImage, onClose }) => {
    ...
    useEffect(() => {
      ...
      window.addEventListener('nudifyHistoryUpdate', handleHistoryUpdate);
      
      return () => {
        window.removeEventListener('nudifyHistoryUpdate', handleHistoryUpdate);
      };
    }, [fetchHistory]);
    ...
    const handleDownloadSelected = async () => {
      ...
      for (let i = 0; i < selectedPredictions.length; i++) {
        ...
        const filename = `nudify_${isVideo ? 'video' : 'image'}_${dateStr}_${timeStr}_${pred.id.slice(0, 8)}.${extension}`;
  ...
  return (
    <div className="min-h-screen bg-black text-gray-100">
      ...
      {/* Sidebar */}
      <NudifySidebar activeMenu={activeMenu} onMenuChange={setActiveMenu} />

      {isHistoryVisible && (
        <NudifyHistorySidebar
